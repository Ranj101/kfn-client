...
class ProducerPublicScreen extends StatefulWidget {
  const ProducerPublicScreen({super.key, required this.id});

  final String id;

  @override
  State<ProducerPublicScreen> createState() => _ProducerPublicScreenState();
}

class _ProducerPublicScreenState extends State<ProducerPublicScreen> {
  ...
  Future<bool> getProducerPageById() async {
    final response = await HttpClient.get('/v1/producers/pages/${widget.id}');
  ...
  Future<bool> getProducts() async {
    ...
    var query =
        'PageIndex=$productCurrentPage&PageSize=8&SortBy=${productSortBySelectedValue ?? 'DateCreated'}&SortDirection=${productSortDirectionValue ?? 'Descending'}&FilterByProducer=${widget.id}';
  ...
  Widget build(BuildContext context) {
    if (loading) {
      return const Scaffold(
        ...
        body: SingleChildScrollView(
          child: Column(
            children: [
              NavBar(currentScreen: 'ProducerPublic'),
              LoadIndicator()
            ],
          ),
        ),
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            const NavBar(currentScreen: 'ProducerPublic'),
            Container(
              child: Padding(
